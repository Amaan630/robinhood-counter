<template>
    <div class="relative overflow-hidden text-left mask-fade">
        <transition-group name="slide-out">
            <span :key="`${before}_before`" class="inline-flex self-center text-center">{{ before }}</span>
            <span
            ...
            <span :key="`${after}_after`" class="inline-flex self-center text-center">{{ after }}</span>
        </transition-group>
    </div>
</template>
    ...
    methods: {
        ...
        continuousEffect() {
            ...
            const updateHeading = () => {
                ...
                this.currentValue = Number((startPrice - (startPrice - endPrice) * easedProgress).toFixed(0));
